...
#### 1. 标签方式改进
- **旧格式**: `#card` + `#deck/xxx` 双标签
- **新格式**: `#card/xxx` 单标签
  - `#card` → Default deck
  - `#card/English` → English deck
  - `#card/物理` → 物理 deck

#### 2. 卡片浏览器重构
- **两级导航结构**:
  - 第一级：Deck 列表视图（显示所有 deck 及其统计信息）
  - 第二级：卡片列表视图（显示特定 deck 的卡片）
- **Deck 统计信息**:
  - 总卡片数、新卡数、已到期数、今天到期数、未来到期数
  - Default deck 排在最前，其他按名称排序
- **复习功能**:
  - "开始复习所有 Deck" 按钮
  - "开始复习此 Deck" 按钮（在卡片列表视图）

#### 3. 核心逻辑修改
- **新增函数**:
  - `extractDeckName()`: 从块的 refs 中提取 deck 名称
  - `calculateDeckStats()`: 计算每个 deck 的统计信息
- **修改函数**:
  - `scanCardsFromTags()`: 使用新的标签提取逻辑
  - `collectReviewCards()`: 为每张卡片添加 deck 字段
  - `startReviewSession()`: 支持按 deck 名称过滤卡片

#### 4. 类型定义扩展
- **修改 `ReviewCard` 类型**: 将 `deck` 字段从可选改为必填
- **新增类型**:
  - `DeckInfo`: 单个 deck 的统计信息
  - `DeckStats`: 全局统计信息

#### 5. 扫描功能问题与调试
- **问题**: 扫描 `#card/xxx` 格式标签时可能无法正确识别
- **解决方案**: 实现多层次扫描策略
  1. 直接查询 `["card"]` 标签
  2. 尝试查询多个可能的标签（`["card", "card/", "card/english", ...]`）
  3. 获取所有块并手动过滤
- **调试功能**: 添加详细的控制台日志，帮助诊断问题

## 🐛 当前问题

### 扫描 `#card/xxx` 格式标签问题
- **现象**: 使用 `#card/xxx` 格式标签的块可能无法被扫描功能识别
- **可能原因**:
  1. Orca API 的 `get-blocks-with-tags` 可能不支持层级标签查询
  2. 标签在数据库中的存储格式可能与预期不同
  3. 标签匹配逻辑可能有误
- **调试方法**:
  1. 查看控制台输出的详细日志
  2. 检查每个块的 refs 结构
...
### 解决方案
已实现多层次扫描策略和详细日志输出，需要用户测试并提供控制台输出以进一步诊断问题。

**下一步**:
1. 解决 `#card/xxx` 格式标签扫描问题（需要用户测试反馈）
2. 支持 deck/标签筛选与排序；增加每日新卡/旧卡上限配置。
3. 在复习界面展示复习统计与历史记录，允许查看/撤销最近评分。
4. 完善错误提示与加载态（队列构建、评分保存）。
5. 在卡片浏览器中添加更多功能（deck 筛选、搜索、批量操作等）。
